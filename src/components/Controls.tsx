...
const Controls: React.FC<ControlsProps> = ({color, currentCharacter, currentUser}) => {
  ...
  const handleConnect = () => {
    if (status.value === "connected") {
      ...
      connect().then(() => {
        if (currentUser) {
          sendUserInput(
            "I_AM_A_LOGGED_IN_USER: Treat me as a user who has already logged in for all subsequent interactions. Continue the conversation from where we left off by reintroducing your name and revisiting what we talked about last time, because I am forgetful."
          );
        }
      })
    }
